// components/management/MuscleTypesManagement.tsx
import React, { useEffect, useState } from 'react';
import { motion } from 'framer-motion';
import { Plus, Edit, Trash, Save, X } from 'lucide-react';
import Button from '../../../components/ui/Button';
import Input from '../../../components/ui/Input';
import { muscleTypeApi } from '../../../lib/api';
import { MuscleType } from '../../../types';
import { formatApiError } from '../../../lib/utils';

const MuscleTypesManagement: React.FC = () => {
  const [items, setItems] = useState<MuscleType[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [isAdding, setIsAdding] = useState(false);
  const [newName, setNewName] = useState('');
  const [editing, setEditing] = useState<MuscleType | null>(null);
  ...
  const addItem = async () => {
    ...
    try {
      await muscleTypeApi.create({ muscleTypeName: newName });
      setNewName('');
      setIsAdding(false);
      fetchItems();
    } catch (e) {
  ...
  const deleteItem = async (id: number) => {
    if (!confirm('האם למחוק את סוג השריר?')) return;
    try {
      await muscleTypeApi.delete(id);
      fetchItems();
    } catch (e) {
      setError(formatApiError(e));
    }
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="bg-white shadow rounded-lg">
        <div className="px-4 py-5 flex justify-between items-center">
          <h2 className="text-xl font-semibold">ניהול סוגי שרירים</h2>
          <Button onClick={() => setIsAdding(true)} icon={<Plus />} disabled={isAdding}>הוסף סוג</Button>
        </div>
        ...
        {isAdding && (
          <div className="px-4 py-3 bg-blue-50 border-y">
            <div className="flex items-center gap-2">
              <Input value={newName} onChange={e => setNewName(e.target.value)} placeholder="שם סוג שריר" fullWidth />
...
export default MuscleTypesManagement;
